...
CREATE POLICY "Management board can manage badges" ON public.badges FOR ALL USING (
...
-- Anonymous reports policies (no auth required)
CREATE POLICY "Anyone can submit anonymous reports" ON public.anonymous_reports FOR INSERT USING (true);
CREATE POLICY "Management board can view anonymous reports" ON public.anonymous_reports FOR SELECT USING (
  public.get_current_user_role() = 'management_board'
);

-- Attendance records policies
CREATE POLICY "Management board can manage attendance" ON public.attendance_records FOR ALL USING (
  public.get_current_user_role() = 'management_board'
);
...
CREATE POLICY "Management board can manage tools" ON public.tools FOR ALL USING (
  public.get_current_user_role() = 'management_board'
);
